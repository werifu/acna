...
import { URI_CATEGORIES_MAP, CATEGORIES, CATEGORIES_URL_MAP } from '@/app/lib/category';
...
export default async function CategoryPage({
  ...
  params: { lang: string; categoryUri: string };
  searchParams?: { page?: string };
}) {
  const resolvedParams = await params;
  const resolvedSearchParams = await searchParams;
  const { lang, categoryUri } = resolvedParams;
  const category = URI_CATEGORIES_MAP[categoryUri as keyof typeof URI_CATEGORIES_MAP];
  ...
  const articles = allArticles.slice((page - 1) * articlesPerPage, page * articlesPerPage);
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="hidden lg:block lg:w-1/5">
            <div className="bg-white border border-gray-400 rounded-lg overflow-hidden sticky top-4">
              ...
              <ul>
                {CATEGORIES.map((cat) => {
                  const catUri = CATEGORIES_URL_MAP[cat as keyof typeof CATEGORIES_URL_MAP];
                  return (
                    <li key={cat} className={`${categoryUri === catUri ? "bg-[#dddddd]" : ""}`}>
                      <Link
                        href={`/${lang}/${catUri}`}
                        className="hover:text-orange-500 px-3 block py-2 flex items-center"
                      >
                        {CATEGORY_I18N[cat as keyof typeof CATEGORY_I18N][lang as keyof typeof CATEGORY_I18N['Articles']]}
                      </Link>
                ...
                <div key={article.slug} className="mb-3">
                  ...
                  >
                    <span
                      ...
                      dangerouslySetInnerHTML={{ __html: TrumpFormater("text-xl font-bold")(article.title) }}
                    ></span>
