...
const CATEGORY_TRANSLATIONS: Record<string, TranslationMapping> = {
  ...
  'Briefings & Statements': { cn: '简报和声明', jp: 'ブリーフィングと声明', kp: '브리핑과 성명' },
...
async function translateText(text: string, targetLang: LangCode): Promise<string> {
  console.log(`[DEBUG] Translating to ${targetLang}, text length: ${text.length} characters`);

  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      console.log(`[DEBUG] API call attempt ${attempt + 1}/${MAX_RETRIES} for ${targetLang}`);
        ...
        messages: [
          {
            ...
            ${Object.entries(FIXED_TRANSLATIONS)
                .map(([key, translations]) => `- "${key}" should be translated as "${translations[targetLang]}"`)
                .join('\n')}
      ...
      console.error(`[DEBUG] Translation error (attempt ${attempt + 1}/${MAX_RETRIES}):`, error.message);
...
async function translateItem(
  ...
  console.log(`[DEBUG] Translating item with slug: ${item.slug} to ${targetLang}`);

  const existing = existingTranslations.find(t => t.slug === item.slug);
  if (existing) {
    console.log(`[DEBUG] Found existing translation for ${item.slug}, skipping`);
  ...
  console.log(`[DEBUG] JSON string for translation, length: ${jsonString.length}`);

  // Translate the entire JSON object at once
  const translatedJsonString = await translateText(jsonString, targetLang);
  console.log(`[DEBUG] Received translated JSON string, length: ${translatedJsonString.length}`);
  ...
  try {
    ...
    if (!translatedItem.title || !translatedItem.category ||
      !translatedItem.date || !translatedItem.slug ||
      !translatedItem.content) {
      throw new Error(`Missing required fields in translated item for ${item.slug}`);
    }

    // Save the updated translations immediately after each successful translation
    existingTranslations.push(translatedItem);
    await fsPromises.writeFile(filePath, JSON.stringify(existingTranslations, null, 2));
    console.log(`[DEBUG] Saved updated translations to ${filePath} after translating ${item.slug}`);

    return translatedItem;
  } catch (error: any) {
    console.error(`[DEBUG] Error parsing translated JSON for ${item.slug}:`, error);
    // Fallback to the original item with predefined category translation
    console.log(`[DEBUG] Falling back to original item with predefined category translation`);

    // Save the fallback translation
    existingTranslations.push(itemToTranslate);
    await fsPromises.writeFile(filePath, JSON.stringify(existingTranslations, null, 2));
    console.log(`[DEBUG] Saved fallback translation to ${filePath} for ${item.slug}`);
...
async function main() {
  ...
  const enContent: ContentItem[] = JSON.parse(readFileSync('../contents/en/contents.json', 'utf-8'));
  ...
  for (const [langCode, langInfo] of Object.entries(LANG_CONFIG)) {
    ...
    console.log(`[DEBUG] Processing language: ${langInfo.name} (${targetLang})`);
    ...
    if (existsSync(filePath)) {
      ...
      console.log(`[DEBUG] Loaded ${existing.length} existing translations for ${targetLang}`);
    } else {
    ...
    const itemsToTranslate = enContent.filter(item => !existing.find(t => t.slug === item.slug));
    console.log(`[DEBUG] Starting translation pool with concurrency: ${CONCURRENCY} for ${itemsToTranslate.length} items`);

    // Use asyncPool to translate concurrently
    for await (const result of asyncPool(CONCURRENCY, itemsToTranslate, async (item: ContentItem) => {
      await translateItem(item, targetLang, existing, filePath);
      console.log(`[DEBUG] Completed translation for ${item.slug} to ${targetLang}`);
      return item.slug;
    })) {
    ...
    console.log(`[DEBUG] Sorted contents by date (newest first) and saved to ${filePath}`);

    console.log(`[DEBUG] Completed all translations for ${targetLang}, total items: ${existing.length}`);
